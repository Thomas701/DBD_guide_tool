...
function Show-LauncherError([string]$Message) {
  try {
    ...
    [System.Windows.MessageBox]::Show($Message, "Build Analyzer", "OK", "Error") | Out-Null
  } catch {
    Write-Error $Message
  }
}
...
try {
  ...
  Show-LauncherError ("Impossible d'installer ou de lancer Build Analyzer.`n`n" + $_.Exception.Message)
  exit 1
}
